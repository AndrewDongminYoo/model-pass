...
1. Verify the existing AIT applicant-entry commits and public-list changes against the product and applicant-entry specifications. Check focused unit, Edge Function, and browser behavior; retain the existing test-first evidence where recorded.
...
Verify with `pnpm test`, `pnpm lint`, `pnpm typecheck`, `pnpm format:check`, `pnpm build`, `pnpm build:web`, focused Playwright and Deno tests, `trunk check --all`, and `git diff --check`.
